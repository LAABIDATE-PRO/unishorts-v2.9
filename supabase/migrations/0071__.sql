...
CREATE FUNCTION public.get_admin_user_list()
 RETURNS TABLE(id uuid, first_name text, last_name text, email text, university_email text, institution_name text, role text, account_status text, last_sign_in_at timestamp with time zone, avatar_url text, film_count bigint, total_views bigint, username text, field_of_study text, short_bio text, phone_number text, created_at timestamp with time zone, join_reason text)
 LANGUAGE sql
 SECURITY DEFINER
